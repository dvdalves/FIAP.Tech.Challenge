...
- **Melhoria**: Integração orientada a eventos usando **RabbitMQ** e **MassTransit**, com resiliência baseada em _Transactional Outbox Pattern_ e tratamento de exceções com _Dead Letter Queue (DLQ)_.

### 6. Cache Distribuído (Redis)

- **Objetivo**: Aliviar a carga de leitura no banco de dados para informações altamente consultadas.
- **Melhoria**: Implementação de cache distribuído em memória com **Redis** e `IDistributedCache` usando _Cache-Aside_ e invalidação na alteração de dados.
