...
public record Placa
{
    public Placa(string valor)
    {
        ...
        var placaFormatada = valor.Trim().ToUpper().Replace("-", "");

        if (!Validar(placaFormatada))
            throw new DominioException(
                $"Placa '{valor}' é inválida. Use o formato clássico (AAA-1234) ou Mercosul (AAA1A23).");
    ...
    private static bool Validar(string placa)
    {
        ...
        var padraoTradicional = @"^[A-Z]{3}\d{4}$";
        var padraoMercosul = @"^[A-Z]{3}\d[A-Z]\d{2}$";
